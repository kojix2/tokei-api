...
module Tokei::Api::Services
  ...
  class TokeiService
    ...
    def self.valid_repo_url?(url : String) : Bool
      ...
      gitlab_https = /^https:\/\/gitlab\.com\/[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+(?:\.git)?$/
      gitlab_ssh = /^git@gitlab\.com:[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+(?:\.git)?$/

      # BitbucketのHTTPSまたはSSH URLのパターン
      bitbucket_https = /^https:\/\/bitbucket\.org\/[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+(?:\.git)?$/
      bitbucket_ssh = /^git@bitbucket\.org:[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+(?:\.git)?$/

      # 一般的なgit URLのパターン
      generic_https = /^https:\/\/[a-zA-Z0-9_.-]+\.[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-\/]+(?:\.git)?$/
      generic_ssh = /^git@[a-zA-Z0-9_.-]+\.[a-zA-Z0-9_.-]+:[a-zA-Z0-9_.-\/]+(?:\.git)?$/
